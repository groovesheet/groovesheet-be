...
try:
    ...
    YouTube = None  # YouTube functionality will be disabled if pytube not installed
import numpy as np
...
def get_yt_audio(link):
    if YouTube is None:
        raise ImportError("pytube is not installed. Install it with: pip install pytube")
    yt = YouTube(link)
