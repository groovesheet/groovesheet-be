param(
    [string]$TEST_MP3 = "D:\Coding Files\GitHub\groovesheet\Baseline_Nirvana_test.mp3"
)
...
if (Test-Path "$PROJECT_ROOT\shared-jobs") {
    Get-ChildItem "$PROJECT_ROOT\shared-jobs" | Where-Object { $_.Name -ne 'uploads' } | ForEach-Object { Remove-Item $_.FullName -Recurse -Force }
}
...
if ($statusResponse.status -eq "completed" -and $statusResponse.download_url) {
    ...
    try {
        ...
        Invoke-WebRequest -Uri $statusResponse.download_url -OutFile $downloadPath -TimeoutSec 30
